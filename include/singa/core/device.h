...
class Device {
 ...
 public:
  ...
  Device(int id,
         int num_executors = 16,
         string scheduler = "sync",
         string vm = "gc-only");
  ...
  void CopyData(Blob* dst, const Blob& src,
  ...
  void Submit(function<void(Context*)> fn,
              const vector<Blob*> read_blobs,
              const vector<Blob*> write_blobs);
  ...
  LibType device_lib() const {
    return device_lib_;
  }
  LibType nn_lib() const {
    return nn_lib_;
  }
