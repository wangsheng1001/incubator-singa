...
#define CHECK(x)                                           \
  if (!(x))                                                \
    singa::LogMessageFatal(__FILE__, __LINE__).stream() << "Check "  \
      "failed: " #x << ' '
...
#define CHECK_NOTNULL(x) \
  ((x) == NULL ? singa::LogMessageFatal(__FILE__, __LINE__).stream() << "Check  notnull: "  #x << ' ', (x) : (x)) // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
  while (false) CHECK(x)
...
#if LOG_CUSTOMIZE
#define LOG_INFO singa::CustomLogMessage(__FILE__, __LINE__)
#else
#define LOG_INFO singa::LogMessage(__FILE__, __LINE__)
#endif
#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL singa::LogMessageFatal(__FILE__, __LINE__)
...
#define LOG_IF(severity, condition) \
  !(condition) ? (void)0 : singa::LogMessageVoidify() & LOG(severity)

#ifdef NDEBUG
#define LOG_DFATAL LOG_ERROR
#define DFATAL ERROR
#define DLOG(severity) true ? (void)0 : singa::LogMessageVoidify() & LOG(severity)
#define DLOG_IF(severity, condition) \
  (true || !(condition)) ? (void)0 : singa::LogMessageVoidify() & LOG(severity)
#else
